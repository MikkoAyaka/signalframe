...
const localeCopy = {
  en: {
    ...
    intro: "Sparx UI is MikkoAyaka's dark interface system for data-rich products, personal workspaces, and command surfaces.",
    ...
    typography: {
      ...
      description: "Sparx UI uses a humanist sans for decisions and a compact mono face for provenance, state, and measurement.",
    ...
    product: {
      ...
      description: "Sparx UI turns visual restraint into a product behavior: make the state legible, expose the next useful action, and reveal detail only when it changes a decision.",
...
const adoptionCopy = {
  en: {
    ...
    componentTitle: "Install Sparx UI as an npm package.",
    ...
    componentSteps: ["Install @mikkoayaka/sparx-ui in the React and Tailwind project.", "Import tokens.css after Tailwind in the global stylesheet.", "Let Tailwind scan the package's lib directory so its utility classes are emitted.", "Import only the primitives needed from @mikkoayaka/sparx-ui; add motion/react only if a product actually renders motion."],
    ...
    skillSteps: ["Copy skills/sparx-ui to your agent's skill directory.", "Start a new agent session so it discovers the skill.", "Invoke $sparx-ui with a concrete interface objective and constraints.", "Let the agent inspect the existing product first; it should preserve identity unless you ask for a full restyle."],
  },
  zh: {
    eyebrow: "\u6307\u5357 / 07",
    ...
    componentLabel: "\u7ec4\u4ef6\u5e93",
    componentTitle: "\u4ee5 npm \u5305\u7684\u65b9\u5f0f\u5b89\u88c5 Sparx UI\u3002",
    componentDescription: "\u516c\u5f00\u7684 React \u5305\u5305\u542b\u539f\u8bed\u3001\u7c7b\u578b\u58f0\u660e\u4e0e\u8bbe\u8ba1 Token\u3002\u5b83\u9700\u8981 React \u4ee5\u53ca\u4f1a\u626b\u63cf\u5305\u5185\u6784\u5efa\u7c7b\u540d\u7684 Tailwind \u6784\u5efa\u6d41\u7a0b\u3002",
    componentSteps: ["\u5728 React \u4e0e Tailwind \u9879\u76ee\u4e2d\u5b89\u88c5 @mikkoayaka/sparx-ui\u3002", "\u5728\u5168\u5c40\u6837\u5f0f\u4e2d\u5c06 tokens.css \u7f6e\u4e8e Tailwind \u4e4b\u540e\u5bfc\u5165\u3002", "\u8ba9 Tailwind \u626b\u63cf\u8be5\u5305\u7684 lib \u76ee\u5f55\uff0c\u4ee5\u4fbf\u8f93\u51fa\u5b83\u4f7f\u7528\u7684\u5de5\u5177\u7c7b\u3002", "\u4ec5\u4ece @mikkoayaka/sparx-ui \u5bfc\u5165\u6240\u9700\u539f\u8bed\uff1b\u53ea\u5728\u4ea7\u54c1\u771f\u6b63\u6e32\u67d3\u52a8\u6548\u65f6\u52a0\u5165 motion/react\u3002"],
    skillLabel: "Agent Skill",
    skillTitle: "\u5728\u5b9e\u73b0\u4e4b\u524d\uff0c\u5148\u5c06\u8fd9\u5957\u7cfb\u7edf\u7684\u51b3\u7b56\u4ea4\u7ed9 Agent\u3002",
    skillDescription: "\u968f\u4ed3\u5e93\u63d0\u4f9b\u7684 Skill \u4f1a\u5c06\u5e03\u5c40\u3001\u5b57\u4f53\u3001\u8868\u9762\u3001\u7ec4\u4ef6\u548c\u52a8\u6548\u89c4\u5219\u5e26\u5165 Agent \u7684\u5de5\u4f5c\u4e0a\u4e0b\u6587\u3002\u5b83\u4e0d\u4f9d\u8d56\u8fd0\u884c\u65f6\uff0c\u4e5f\u4e0d\u662f\u7ec4\u4ef6\u5305\u3002",
    skillSteps: ["\u5c06 skills/sparx-ui \u590d\u5236\u5230\u4f60\u7684 Agent Skill \u76ee\u5f55\u3002", "\u542f\u52a8\u4e00\u4e2a\u65b0\u7684 Agent \u4f1a\u8bdd\uff0c\u8ba9\u5b83\u53d1\u73b0\u8be5 Skill\u3002", "\u7528\u660e\u786e\u7684\u754c\u9762\u76ee\u6807\u4e0e\u7ea6\u675f\u6761\u4ef6\u8c03\u7528 $sparx-ui\u3002", "\u8ba9 Agent \u5148\u68c0\u67e5\u65e2\u6709\u4ea7\u54c1\uff1b\u9664\u975e\u4f60\u660e\u786e\u8981\u6c42\u5168\u9762\u91cd\u5851\uff0c\u5b83\u5e94\u4fdd\u7559\u4ea7\u54c1\u672c\u8eab\u7684\u8bc6\u522b\u3002"],
  },
} as const;
...
const exampleCopy = {
  en: {
    eyebrow: "Examples / 05",
    title: "One system, three product postures.",
...
const designLineage = [
  ["Linear", "Operational restraint", "Shared: a quiet chrome, high information density, earned motion, and state-led work. Divergence: Sparx UI uses a stronger red signal and a more spatial data canvas."],
  ["Apple Human Interface Guidelines", "Clarity and deference", "Shared: content leads while the interface recedes. Sparx UI translates that principle to dense dark workspaces rather than a platform-native visual language."],
  ["IBM Carbon", "Systematic trust", "Shared: reusable primitives, explicit states, and accessibility as part of the system contract. Sparx UI keeps the system smaller and more editorial in tone."],
] as const;
...
const zhText: Record<string, string> = {
  ...
  "Foundations / 03": "基础 / 03", "Motion and spatial depth": "动效与空间层次", "Sparx UI treats motion as a state transition. It should clarify origin, hierarchy, or completion, never decorate an idle screen.": "Sparx UI 将动效视为状态过渡：它应解释来源、层级或完成，而不是装饰静止页面。",
  "Rest before motion": "先有静态，再有动效", "One depth plane": "单一深度平面", "Timing ladder": "时长阶梯", "Short for feedback. Longer for spatial change.": "反馈要短，空间变化可以更长。",
  ...
  "Spatial handoff": "空间交接", "Direct manipulation": "直接操控", "Interruption policy": "中断策略", "Use motion when": "适合使用动效", "Do not animate": "不应使用动效",
  ...
  "Components / 03": "组件 / 03", "Portable primitives": "可移植原语", "All components are source-owned under src/sparx-ui and rendered by this documentation site directly.": "所有组件均由 src/sparx-ui 源码维护，并由本站直接渲染。", "Component": "组件",
  "Components / 04": "组件 / 04", "Data and operational states": "数据与运行状态", "Patterns / 04": "模式 / 04", "Composition rules": "组合规则", "Guidance / 06": "指南 / 06", "Write and compose with intent": "以明确意图编写与组合", "Content hierarchy": "内容层级", "State and feedback": "状态与反馈", "Do": "推荐", "Avoid": "避免", "Adoption checklist": "接入检查清单",
  ...
  "Toggle documentation navigation": "切换文档导航", "Sparx UI documentation": "Sparx UI 文档", "v0.1 / portable core": "v0.1 / 可移植核心", "2m ago": "2 分钟前", "14m ago": "14 分钟前",
  ...
  "Design references and boundaries": "设计参照与边界", "Sparx UI shares a modern product-workspace grammar with respected systems, while keeping its own visual and operational constraints.": "Sparx UI 与成熟系统共享现代产品工作台的设计语法，同时保留自身的视觉与操作约束。", "Operational restraint": "克制的运行界面", "Shared: a quiet chrome, high information density, earned motion, and state-led work. Divergence: Sparx UI uses a stronger red signal and a more spatial data canvas.": "共性：安静的框架、高信息密度、有意义的动效和由状态引导的工作流。差异：Sparx UI 使用更鲜明的红色信号与更具空间感的数据画布。", "Clarity and deference": "清晰与退让", "Shared: content leads while the interface recedes. Sparx UI translates that principle to dense dark workspaces rather than a platform-native visual language.": "共性：内容主导，界面退让。Sparx UI 将这一原则用于数据密集的深色工作台，而非平台原生的视觉语言。", "Systematic trust": "系统化的可信度", "Shared: reusable primitives, explicit states, and accessibility as part of the system contract. Sparx UI keeps the system smaller and more editorial in tone.": "共性：可复用原语、明确状态，以及无障碍作为系统约定的一部分。Sparx UI 保持更小的系统规模与更具编辑感的语气。",
};

const I18nContext = createContext<(value: string) => string>((value) => value);

const code = {
  panel: `<SparxPanel tone="default" className="p-6">
  <SparxSectionHeading
  ...
  chip: `<SparxChip tone="signal">
  <CircleDot className="h-3 w-3" /> Live
</SparxChip>`,
  icon: `<SparxIconButton aria-label="Open command center">
...
/>`,
  ...
  meta="2m ago"
  trailing={<SparxChip tone="success">Ready</SparxChip>}
/>`,
  motion: `import { motion } from "motion/react";
import { sparxMotion } from "@mikkoayaka/sparx-ui";

<motion.div {...sparxMotion.reveal}>
  <SparxPanel className="p-6">...</SparxPanel>
</motion.div>`,
...
function ComponentSpec({ title, description, preview, children }: { title: string; description: string; preview: ReactNode; children: string }) {
  ...
  return (
    <SparxPanel interactive={false} tone="quiet" className="overflow-visible">
        ...
        <main id="top" className="min-w-0 px-5 py-10 sm:px-8 lg:px-14 lg:py-16">
          <div className="mx-auto max-w-5xl space-y-24">
            <section className="relative overflow-hidden border-b border-white/[0.08] pb-16">
              ...
              <SparxChip tone="signal"><CircleDot className="h-3 w-3" /> {copy.eyebrow}</SparxChip>
              ...
              <div className="mt-9 flex flex-wrap items-center gap-3">
                ...
                <a href="#components" className="sparx-docs-link inline-flex items-center gap-2 px-2 py-2.5 text-sm">{copy.inspect} <MoveUpRight className="h-4 w-4" /></a>
              </div>
            </section>

            <DocSection id="principles">
              <SparxSectionHeading eyebrow={t("Foundations / 01")} title={t("Principles")} description={t("The rules that make the system recognizable across focused workspaces and data-rich interfaces.")} />
            ...
            <DocSection id="foundations">
              <SparxSectionHeading eyebrow={t("Foundations / 02")} title={t("Color, depth, and type")} description={t("Neutral surfaces keep attention available for one signal color. The mono face labels systems; the sans face carries content.")} />
              <div className="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
                <SparxPanel interactive={false} tone="quiet" className="p-6">
            ...
            <DocSection id="typography">
              <SparxSectionHeading eyebrow={copy.typography.eyebrow} title={copy.typography.title} description={copy.typography.description} />
              <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
                ...
                <SparxPanel interactive={false} tone="quiet" className="overflow-hidden">
            ...
            <DocSection id="motion">
              <SparxSectionHeading eyebrow={t("Foundations / 03")} title={t("Motion and spatial depth")} description={t("Sparx UI treats motion as a state transition. It should clarify origin, hierarchy, or completion, never decorate an idle screen.")} />
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <SparxPanel interactive={false} tone="quiet" className="p-6"><p className="font-mono text-xs uppercase tracking-[0.18em] text-emerald-300">{t("Use motion when")}</p><ul className="mt-5 space-y-2 text-sm leading-6 text-neutral-300"><li>{t("It connects a trigger to a changed state.")}</li><li>{t("It shows where content came from or where it went.")}</li><li>{t("It explains progress, completion, or replacement.")}</li><li>{t("The static state still communicates the full result.")}</li></ul></SparxPanel>
                <SparxPanel interactive={false} tone="quiet" className="p-6"><p className="font-mono text-xs uppercase tracking-[0.18em] text-red-300">{t("Do not animate")}</p><ul className="mt-5 space-y-2 text-sm leading-6 text-neutral-300"><li>{t("Text that is frequently scanned for updates.")}</li><li>{t("Every card on route entry or every list item on filter change.")}</li><li>{t("Values whose stable alignment matters more than novelty.")}</li><li>{t("Hidden content merely because it can be made to move.")}</li></ul></SparxPanel>
              </div>
              <MotionLab copy={copy.motionLab} />
            </DocSection>

            <DocSection id="product-design">
              <SparxSectionHeading eyebrow={copy.product.eyebrow} title={copy.product.title} description={copy.product.description} />
              ...
              <SparxPanel interactive={false} tone="command" className="p-6">
                <div className="max-w-3xl">
                  <p className="font-mono text-xs uppercase tracking-[0.18em] text-red-400">{t("Design references and boundaries")}</p>
                  <p className="mt-3 text-lg font-medium tracking-tight text-white">{t("Sparx UI shares a modern product-workspace grammar with respected systems, while keeping its own visual and operational constraints.")}</p>
                </div>
            ...
            <DocSection id="accessibility">
              <SparxSectionHeading eyebrow={t("Foundations / 04")} title={t("Accessible by default")} description={t("A dark system gains quality from clarity, not from low contrast. Every signal needs a non-color equivalent.")} />
              <div className="grid gap-4 md:grid-cols-2">
                <SparxPanel interactive={false} tone="quiet" className="p-6"><div className="flex items-start gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-emerald-500/20 bg-emerald-500/10 text-emerald-300"><ShieldCheck className="h-5 w-5" /></span><div><h3 className="text-sm font-medium text-white">{t("Required behavior")}</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-neutral-400"><li>{t("Every icon-only action has an accessible label.")}</li><li>{t("Focus rings remain visible on transparent controls.")}</li><li>{t("Status combines color with label, icon, or position.")}</li><li>{t("Progress has a programmatic value and name.")}</li></ul></div></div></SparxPanel>
                <SparxPanel interactive={false} tone="quiet" className="p-6"><div className="flex items-start gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-amber-500/20 bg-amber-500/10 text-amber-200"><MonitorSmartphone className="h-5 w-5" /></span><div><h3 className="text-sm font-medium text-white">{t("Responsive baseline")}</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-neutral-400"><li>{t("Keep an action's touch target at least 40px square.")}</li><li>{t("Collapse rails before shrinking readable content.")}</li><li>{t("Allow code blocks to scroll locally, never the page.")}</li><li>{t("Preserve reading order when grids reflow.")}</li></ul></div></div></SparxPanel>
              </div>
            </DocSection>

            <DocSection id="components">
              <SparxSectionHeading eyebrow={t("Components / 03")} title={t("Portable primitives")} description={t("All components are source-owned under src/sparx-ui and rendered by this documentation site directly.")} />
              <div className="space-y-5">
                <ComponentSpec title="SparxPanel" description="The primary surface. It combines a translucent neutral base, hairline border, input-led radial signal, and a single depth layer." preview={<SparxPanel className="w-full max-w-sm p-5"><p className="font-mono text-xs uppercase tracking-[0.18em] text-red-400">{t("Realtime")}</p><p className="mt-4 text-2xl font-semibold tracking-tight text-white">{t("Noisy data. Quiet frame.")}</p><p className="mt-2 text-sm leading-6 text-neutral-400">{t("Hover to reveal the signal field.")}</p></SparxPanel>}>{code.panel}</ComponentSpec>
                <ComponentSpec title="SparxChip" description="A compact status label. Use a semantic tone for live state, risk, success, or a quiet metadata label. Do not use it as primary navigation." preview={<div className="flex flex-wrap justify-center gap-2"><SparxChip tone="signal"><CircleDot className="h-3 w-3" /> {t("Live")}</SparxChip><SparxChip tone="success"><Check className="h-3 w-3" /> {t("Ready")}</SparxChip><SparxChip tone="warning">{t("Review")}</SparxChip><SparxChip>{t("Draft")}</SparxChip></div>}>{code.chip}</ComponentSpec>
                <ComponentSpec title="SparxIconButton" description="A single-action control for dense toolbars. The icon must have an accessible label; an active state means the tool is currently selected, not merely hovered." preview={<div className="flex gap-3"><SparxIconButton aria-label={t("Open command center")} active><Command className="h-4 w-4" /></SparxIconButton><SparxIconButton aria-label={t("Copy component code")}><Copy className="h-4 w-4" /></SparxIconButton><SparxIconButton aria-label={t("Open layers")}><Layers3 className="h-4 w-4" /></SparxIconButton></div>}>{code.icon}</ComponentSpec>
                <ComponentSpec title="SparxSectionHeading" description="A divider that establishes scanning hierarchy without enclosing a new card. Prefer it over nested panels when content is already inside a surface." preview={<div className="w-full max-w-sm"><SparxSectionHeading eyebrow={t("System / active")} title={t("Command history")} description={t("A title, a quiet explanation, and optional actions share one line of hierarchy.")} action={<SparxChip tone="signal">{t("04 events")}</SparxChip>} /></div>}>{`<SparxSectionHeading\n  eyebrow="System / active"\n  title="Command history"\n  description="A title, a quiet explanation, and optional actions."\n/>`}</ComponentSpec>
              </div>
            </DocSection>

            <DocSection id="data-components">
              <SparxSectionHeading eyebrow={t("Components / 04")} title={t("Data and operational states")} description={t("These primitives express the pieces most often repeated in dashboards, portfolios, and review tools without forcing a product-specific layout.")} />
              <div className="space-y-5">
                <ComponentSpec title="SparxMetric" description="A metric is a primary data point, not a decorative number. Pair it with a stable label; use a color tone only when the value itself carries semantic state." preview={<div className="grid w-full max-w-sm grid-cols-2 gap-7"><SparxMetric label={t("Qualified sessions")} value="18.4k" delta="+12.6%" tone="success" detail={t("Compared with last week")} /><SparxMetric label={t("Median latency")} value="164ms" delta="-21ms" tone="signal" detail={t("p95 response time")} /></div>}>{code.metric}</ComponentSpec>
                <ComponentSpec title="SparxProgress" description="Progress is for bounded completion, coverage, or capacity. Do not use it to imply a percentage when a task has no measurable end state." preview={<div className="w-full max-w-sm space-y-5"><SparxProgress label={t("Verification coverage")} value="76%" percent={76} tone="success" /><SparxProgress label={t("Migration readiness")} value="42%" percent={42} tone="warning" /><SparxProgress label={t("Active runtime")} value="98%" percent={98} /></div>}>{code.progress}</ComponentSpec>
                <ComponentSpec title="SparxDataRow" description="A dense row preserves scan order: identity first, descriptive context second, quiet metadata third, and actionable or semantic state at the edge." preview={<div className="w-full max-w-sm"><SparxDataRow leading={<Archive className="h-4 w-4" />} label={t("Production release")} description={t("Ready to publish")} meta={t("2m ago")} trailing={<SparxChip tone="success">{t("Ready")}</SparxChip>} /><SparxDataRow leading={<Activity className="h-4 w-4" />} label={t("Quality gate")} description={t("Awaiting review")} meta={t("14m ago")} trailing={<SparxChip tone="warning">{t("Review")}</SparxChip>} /></div>}>{code.row}</ComponentSpec>
                <ComponentSpec title="SparxEmptyState" description="An empty state explains what is absent, why it matters, and the single next action when one exists. It occupies one quiet surface rather than creating a modal interruption." preview={<SparxPanel interactive={false} tone="quiet" className="w-full max-w-sm"><SparxEmptyState icon={<Inbox className="h-5 w-5" />} title={t("No deployments yet")} description={t("Connect a project or publish a first release to populate this stream.")} action={<button type="button" className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-100">{t("Connect project")}</button>} /></SparxPanel>}>{`<SparxEmptyState\n  icon={<Inbox className="h-5 w-5" />}\n  title="No deployments yet"\n  description="Connect a project to populate this stream."\n/>`}</ComponentSpec>
                <ComponentSpec title="SparxToolbar" description="Toolbars group adjacent controls, not every page action. A toolbar should be compact, keyboard reachable, and visually subordinate to the content it manipulates." preview={<SparxToolbar label={t("Canvas tools")}><SparxIconButton aria-label={t("Open filters")} active><ListFilter className="h-4 w-4" /></SparxIconButton><SparxIconButton aria-label={t("Open display settings")}><SlidersHorizontal className="h-4 w-4" /></SparxIconButton><span className="mx-1 h-5 w-px bg-white/10" /><SparxChip>{t("Compact")}</SparxChip></SparxToolbar>}>{`<SparxToolbar label="Canvas tools">\n  <SparxIconButton aria-label="Open filters">...</SparxIconButton>\n  <SparxIconButton aria-label="Open display settings">...</SparxIconButton>\n</SparxToolbar>`}</ComponentSpec>
              </div>
            </DocSection>

            <DocSection id="patterns">
              <SparxSectionHeading eyebrow={t("Patterns / 04")} title={t("Composition rules")} description={t("These patterns preserve the system's character when primitives become a product interface.")} />
              ...
              <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
                <SparxPanel interactive={false} tone="quiet" className="p-5 sm:p-6">
                  ...
                  <div className="mt-5 flex flex-col gap-5 border-b border-white/[0.07] pb-5 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <SparxToolbar label={t("Revenue workspace actions")}><SparxIconButton aria-label={t("Open workspace filters")}><ListFilter className="h-4 w-4" /></SparxIconButton><SparxIconButton aria-label={t("Open workspace settings")}><SlidersHorizontal className="h-4 w-4" /></SparxIconButton></SparxToolbar>
                  </div>
              ...
              <SparxPanel interactive={false} tone="quiet" className="p-5 sm:p-6">
                <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="font-mono text-xs uppercase tracking-[0.18em] text-red-400">{t("Layout / adaptive data canvas")}</p><h3 className="mt-3 text-lg font-medium text-white">{t("Span by information density, not visual symmetry.")}</h3></div><SparxChip tone="quiet"><Rows3 className="h-3 w-3" /> {t("12-column desktop / single-column mobile")}</SparxChip></div>
                <div className="mt-6 grid gap-3 md:grid-cols-6"><div className="md:col-span-4"><SparxPanel interactive={false} tone="default" className="p-4"><div className="flex items-center justify-between"><SparxMetric label={t("Qualified pipeline")} value="$482k" delta="+8.4%" tone="success" /><BarChart3 className="h-8 w-8 text-red-400/60" /></div><div className="mt-5 grid grid-cols-3 gap-2"><span className="h-10 rounded-lg bg-red-500/15" /><span className="h-10 rounded-lg bg-red-500/10" /><span className="h-10 rounded-lg bg-red-500/5" /></div></SparxPanel></div><div className="md:col-span-2"><SparxPanel interactive={false} tone="quiet" className="p-4"><Gauge className="h-5 w-5 text-amber-200" /><p className="mt-5 text-sm font-medium text-white">{t("Review queue")}</p><p className="mt-1 text-xs text-neutral-500">{t("14 items waiting")}</p></SparxPanel></div></div>
              </SparxPanel>
            </DocSection>
            ...
            <DocSection id="guidance">
              <SparxSectionHeading eyebrow={t("Guidance / 06")} title={t("Write and compose with intent")} description={t("The system is portable because it defines decisions, not because every screen looks the same. Use these rules before introducing a new primitive.")} />
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <SparxPanel interactive={false} tone="quiet" className="p-6"><p className="font-mono text-xs uppercase tracking-[0.18em] text-emerald-300">{t("Do")}</p><ul className="mt-5 space-y-3 text-sm leading-6 text-neutral-300"><li>{t("Give the page one obvious data or identity anchor.")}</li><li>{t("Use a border or tonal shift before introducing a shadow.")}</li><li>{t("Let nearby controls share a toolbar; keep global actions outside it.")}</li><li>{t("Use the grid to align information, not to decorate empty space.")}</li></ul></SparxPanel>
              ...
              <SparxSectionHeading eyebrow={adoption.eyebrow} title={adoption.title} description={adoption.description} />
              <div className="grid gap-4 xl:grid-cols-2">
                <SparxPanel interactive={false} tone="command" className="min-w-0 p-6">
                  ...
                  <div className="mt-6 overflow-hidden rounded-xl border border-white/[0.07]"><CodeBlock language="bash" label="SHELL" code={`npm install @mikkoayaka/sparx-ui`} /></div>
                  <div className="mt-3 overflow-hidden rounded-xl border border-white/[0.07]"><CodeBlock language="css" label="SRC / INDEX.CSS" code={`@import "tailwindcss";\n@source "../node_modules/@mikkoayaka/sparx-ui/lib";\n@import "@mikkoayaka/sparx-ui/tokens.css";`} /></div>
                  <div className="mt-3 overflow-hidden rounded-xl border border-white/[0.07]"><CodeBlock label="TSX" code={`import { SparxPanel, SparxToolbar } from "@mikkoayaka/sparx-ui";`} /></div>
                </SparxPanel>
                <SparxPanel interactive={false} tone="quiet" className="min-w-0 p-6">
                  ...
                  <div className="mt-6 overflow-hidden rounded-xl border border-white/[0.07]"><CodeBlock language="bash" label="SHELL" code={`# Codex\nmkdir -p "\${CODEX_HOME:-$HOME/.codex}/skills"\ncp -R ./skills/sparx-ui "\${CODEX_HOME:-$HOME/.codex}/skills/"\n\n# Claude Code\nmkdir -p .claude/skills\ncp -R ./skills/sparx-ui .claude/skills/`} /></div>
                  <div className="mt-3 overflow-hidden rounded-xl border border-white/[0.07]"><CodeBlock language="bash" label="PROMPT" code={`Use $sparx-ui to refine the billing workspace. Preserve the existing product identity, make the primary decision obvious, and verify desktop, mobile, keyboard, and state behavior.`} /></div>
                </SparxPanel>
              </div>
            </DocSection>

            <footer className="flex flex-col gap-3 border-t border-white/[0.08] py-10 font-mono text-xs uppercase tracking-[0.16em] text-neutral-600 sm:flex-row sm:items-center sm:justify-between"><span>Sparx UI / MikkoAyaka</span><span>{copy.footer}</span></footer>
          </div>
        </main>
      </div>
    </div>
...
function Navigation({ locale, className = "" }: { locale: Locale; className?: string }) {
  ...
  return (
    <nav className={`space-y-7 ${className}`} aria-label={locale === "zh" ? zhText["Sparx UI documentation"] : "Sparx UI documentation"}>
      {navigation.map(([label, items]) => (
        <div key={label}>
          <p className="mb-3 font-mono text-xs uppercase tracking-[0.18em] text-neutral-600">{label}</p>
          <div className="space-y-2">{items.map(([item, anchor]) => <a key={item} href={`#${anchor}`} className="sparx-docs-link block text-sm">{item}</a>)}</div>
        </div>
      ))}
    </nav>
  );
...
function Pattern({ icon, title, body }: { icon: ReactNode; title: string; body: string }) {
  const t = useContext(I18nContext);
  return <SparxPanel interactive={false} tone="quiet" className="p-6"><span className="grid h-9 w-9 place-items-center rounded-xl border border-red-500/20 bg-red-500/10 text-red-300">{icon}</span><h3 className="mt-7 text-base font-medium text-white">{t(title)}</h3><p className="mt-3 text-sm leading-6 text-neutral-400">{t(body)}</p></SparxPanel>;
}

function RuleCard({ index, title, body }: { index: string; title: string; body: string }) {
  const t = useContext(I18nContext);
  return <SparxPanel interactive={false} tone="quiet" className="p-6"><span className="font-mono text-xs tracking-[0.18em] text-red-400">{index}</span><h3 className="mt-5 text-base font-medium text-white">{t(title)}</h3><p className="mt-3 text-sm leading-6 text-neutral-400">{t(body)}</p></SparxPanel>;
}
...
function MotionRecipe({ trigger, preset, duration, output }: { trigger: string; preset: string; duration: string; output: string }) {
  const t = useContext(I18nContext);
  return <div className="grid gap-2 py-4 sm:grid-cols-[0.8fr_0.45fr_0.45fr_1.4fr] sm:items-center"><span className="text-sm text-white">{t(trigger)}</span><SparxChip tone={preset === "spatial" ? "signal" : "quiet"}>{preset}</SparxChip><span className="font-mono text-xs uppercase tracking-[0.12em] text-neutral-500">{duration}</span><span className="text-xs leading-5 text-neutral-400">{t(output)}</span></div>;
}
...
function GuidanceState({ tone, title, body }: { tone: "signal" | "success" | "warning" | "quiet"; title: string; body: string }) {
  const t = useContext(I18nContext);
  return <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4"><SparxChip tone={tone}>{t(title)}</SparxChip><p className="mt-3 text-xs leading-5 text-neutral-400">{t(body)}</p></div>;
}
